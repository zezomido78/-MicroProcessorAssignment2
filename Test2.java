...
public class Test2 {
    ...
    private  static BigInteger mont(BigInteger aa, BigInteger bb, BigInteger N,BigInteger R, BigInteger NN){
        ...
        if(t.compareTo(N)>0)
            t= t.subtract(N);
        BigInteger result =new BigInteger(aa.toString()).multiply(bb).multiply(R.modInverse(N)).mod(N);
        if(result.compareTo(t) != 0 )
            System.err.println("Error in sasa montgomery");
    ...
    private static BigInteger modExp2(BigInteger a, BigInteger exponent, BigInteger N,BigInteger R,BigInteger NN) {
        ...
        BigInteger aa,bb;
        result=result.multiply(R).mod(N);
        bb=a.multiply(R).mod(N);

        int expBitlength = exponent.bitLength();
        for (int i = expBitlength - 2; i >= 0; i--) {

            result=mont(result,result,N,R,NN);

            if (exponent.testBit(i)) {
                result=mont(result,bb,N,R,NN);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        BigInteger NN=modulus.modInverse(R).negate();
