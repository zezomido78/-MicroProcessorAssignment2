import java.math.BigInteger;
import java.util.Random;

/**
 * Disclaimer:
 * This code is for illustration purposes and shall never be used in any implementation in practice.
 */

public class PlainRSADemo {



    private static BigInteger modPowerTwo(BigInteger N,BigInteger R){
        return         N.and(R.subtract(BigInteger.ONE));

    }
    /*
    Montogomer Multiplication
    INPUT : a , b (in montomery representation)
           N , R , NDash = -N^-1 mod R

   OUTPUT : (a * b) R^-1 mod N


     */
    private static BigInteger montogomeryMultiply (BigInteger a,BigInteger b,BigInteger R,int RBitLength,BigInteger N,BigInteger NDash){
        BigInteger t = a.multiply(b);
        BigInteger m =  modPowerTwo( t.multiply(NDash),R);
         t = (t.add(m.multiply(N))).shiftRight(RBitLength-1);
        if(t.compareTo(N) > 0)
          return t.subtract(N);
        else
            return t;
    }


    private static BigInteger encrypt(BigInteger m, BigInteger e, BigInteger modulus) {
        return modExp(m, e, modulus);
    }

    private static BigInteger decrypt(BigInteger c, BigInteger d, BigInteger modulus) {
        return modExp(c, d, modulus);
    }
    private static BigInteger decryptMontgomery(BigInteger c ,BigInteger d , BigInteger modulus){
        return modExpMontgomery(c,d,modulus);
    }
    private static BigInteger encryptMontgomery(BigInteger c ,BigInteger d , BigInteger modulus){
        return modExpMontgomery(c,d,modulus);
    }

    private static BigInteger modExp(BigInteger a, BigInteger exponent, BigInteger N) {
        // Note: This code assumes the most significant bit of the exponent is 1, i.e., the exponent is not zero.
        long start = System.nanoTime();

        BigInteger result = new BigInteger(a.toString());
        int expBitlength = exponent.bitLength();
        for (int i = expBitlength - 2; i >= 0; i--) {
            result = result.multiply(result).mod(N);
            if (exponent.testBit(i)) {
                result = result.multiply(a).mod(N);
            }
        }
        return result;
    }
    private static BigInteger modExpMontgomery(BigInteger a, BigInteger exponent, BigInteger N) {

        int len = N.bitLength();
        BigInteger R = BigInteger.ONE.shiftLeft(len);
        int expBitlength = exponent.bitLength();
        BigInteger NDash = N.modInverse(R).negate();
      //  BigInteger NDash=BigInteger.ZERO.subtract(N.modInverse(R));   //N'= -1 * N_Inverse


        BigInteger result = a.multiply(R).mod(N);
        BigInteger aa=result;
        for (int i = expBitlength - 2; i >= 0; i--) {
            result = montogomeryMultiply(result,result,R,len+1,N,NDash);

            if (exponent.testBit(i)) {

                result =montogomeryMultiply(result,aa,R,len+1,N,NDash);
            }

        }


        return  result.multiply(R.modInverse(N)).mod(N);


    }

    public static void main(String[] args) {

        // RSA modulus
        BigInteger modulus = new BigInteger(
                "",
                16);

        // private exponent
        BigInteger d = new BigInteger(
                "",
                16);

        // public exponent (just provided for illustration. The focus in this assignment is on the decryption process)
        long normTime=0;
        long montTime=0;
        int nTrials=100;
        for(int i=0;i<nTrials;i++) {
            BigInteger e = new BigInteger("10001", 16);

            Random rnd = new Random();
            BigInteger m = new BigInteger(modulus.bitLength() - 1, rnd);
            BigInteger c = encrypt(m, e, modulus);

            System.out.println("Original message = " + m.toString(16));
            System.out.println("Ciphertext = " + c.toString(16));


            long start = System.nanoTime();
            BigInteger m2 = decrypt(c, d, modulus);
            long elapsedTime = System.nanoTime() - start;
            normTime += elapsedTime/nTrials;
            System.out.println("Decrypted message From Normal = " + m2.toString(16));
            System.out.println("Time taken in microseconds for normal: " + elapsedTime / 1000);
            start = System.nanoTime();
            m2 = decryptMontgomery(c, d, modulus);
            elapsedTime = System.nanoTime() - start;
            montTime += elapsedTime/nTrials;
            System.out.println("Decrypted message From Montogomery= " + m2.toString(16));
            System.out.println("Time taken in microseconds for montogomery: " + elapsedTime / 1000);

            if (!m.equals(m2)) {
                System.err.println("There is an error.");
            }
        }
        double improvPercent = ((double) normTime - (double)  montTime ) / (double) normTime;
        System.out.println("Testing with "+nTrials+" Improvment % = "+improvPercent*100);










    }

}